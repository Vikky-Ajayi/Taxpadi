...
router.post("/calculate", async (req, res) => {
  ...
  // Verify statement belongs to the authenticated user before calculating tax on its data
  const [ownedStmt] = await db.select({ id: statementsTable.id })
    ...
    .limit(1);
  if (!ownedStmt) { res.status(404).json({ error: "Statement not found" }); return; }
